...
Please refer to the **[Install and Configure Pico C/C++ Development Environment Tutorial](/docs/Raspberry-Pi-Pico/Tutorials/C-Tutorials/index.md)** to download and install the Pico VSCode.
...
|     02_CAM       |             Display camera live feed on the LCD screen          |      -       |
|     03_ES8311    |              Development board audio test program              |      -       |
|     04_FatFs     |     FAT file system, TF card supports SPI/SDIO communication    |      -       |
...
![VSCode-Example-1](./images/VSCode-Example-1.webp)
</div>

The character fonts that the GUI depends on are located in the directory: `c\lib\Fonts`

<div style={{maxWidth:800}}> 
![VSCode-Example-2](./images/VSCode-Example-2.webp)
</div>
...
- Image Rotation: Set the rotation angle for the selected image. It is best to use this after `Paint_SelectImage()`. You can choose rotation angles of 0, 90, 180, or 270 degrees.
    ...
    <div style={{maxWidth:800}}> 
    ![VSCode-Example-3](./images/VSCode-Example-3.webp)
    </div>
    :::
...
- Import and compile the 01_GUI project using VS Code. After compilation, flash the .uf2 file from the `build` directory, or directly flash 01_GUI.uf2 from the `uf2` directory for quick verification.
    ...
    ![VSCode-Example-4](./images/VSCode-Example-4.webp)
    </div>
...
- Uses PIO-emulated DVP to communicate with the OV5640 for camera data capture.

**Hardware Connection**

- Connect the OV5640 camera.
- Connect the board to the computer using a USB cable

**Code Analysis**

- `init_cam()`: Initialize the camera.
- `config_cam_buffer()`: Configure the camera buffer.
- `start_cam()`: Start the camera.
- `LCD_3IN5_Display()`: Display the camera feed on the LCD screen.

**Operation Result**

- Use VS Code to import and compile the 02_CAM project. After compilation is complete, flash the uf2 file in the `build` directory, or directly flash the 02_CAM.uf2 file in the `uf2` directory for quick verification.
- After flashing, the LCD displays the image captured by the camera.
...
- LVGL Display Refresh Rate Setting
    ...
    Setting: In `lv_conf.h`, you can also set the display buffer refresh period. Modify this definition to change the screen refresh time.
    ```
...
- LVGL Create Tile
    ...
    Function: A Tile view is a container object whose elements (called tiles) can be arranged in a grid. Users can navigate between tiles by swiping. Call `lv_tileview_add_tile(tileview, row_id, col_id, dir)` to create a new tile at row `row_id` and column `col_id`. The `dir` can be `LV_DIR_LEFT/RIGHT/TOP/BOTTOM/HOR/VER/ALL` or a numeric value, allowing navigation to adjacent tiles in the specified direction via swiping.
    ```c
...
- LVGL Widget Alignment and Positioning
    ...
    ```
    <div style={{maxWidth:800}}> 
    ![VSCode-Example-5](./images/VSCode-Example-7.webp)
    </div>
...
- Use VS Code to import and compile the 05_LVGL project. After compilation is complete, flash the uf2 file in the `build` directory, or directly flash the 05_LVGL.uf2 file in the `uf2` directory for quick verification.

    <div style={{maxWidth:400}}> 
    ![Wiring Diagram](./images/VSCode-Example-8.webp)
    </div>
