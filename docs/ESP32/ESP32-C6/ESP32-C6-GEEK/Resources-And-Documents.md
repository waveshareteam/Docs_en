---
sidebar_position: 1
title: Resources
slug: /ESP32-C6-GEEK/Resources-And-Documents
product_serial:
  - ESP32
product_model:
  - ESP32-C6-GEEK
---

<!-- Image Reference -->

# Resources

## **1. Hardware Resources**

**Development Board Design File**

- [Schematic](https://files.waveshare.com/wiki/ESP32-C6-GEEK/ESP32-C6-GEEK.pdf)

## **2. Technical Manuals**

- **Official ESP32-C6 Chip Manuals**
  - Datasheets: [Chinese Version](https://documentation.espressif.com/esp32-c6_datasheet_cn.pdf) | [English Version](https://documentation.espressif.com/esp32-c6_datasheet_en.pdf)
  - Technical Manuals: [Chinese Version](https://documentation.espressif.com/esp32-c6_technical_reference_manual_cn.pdf) | [English Version](https://documentation.espressif.com/esp32-c6_technical_reference_manual_en.pdf)

- **Datasheet**
  - [ST7789P3 Datasheet](https://files.waveshare.com/wiki/common/ST7789P3.pdf)

## **3. Demo**

- **Demo (ESP-IDF and Arduino)**: [ESP32-C6-GEEK_Demo.zip](https://files.waveshare.com/wiki/ESP32-C6-GEEK/ESP32-C6-GEEK-Demo.zip)

## **4. Software Tools**

- Debugging Tools:
  - [Serial Port Debug Assistant](https://files.waveshare.com/wiki/common/Sscom5.13.1.zip)
  - [Network Debug Assistant](https://files.waveshare.com/wiki/common/NetAssist.zip)
  - [iOS Bluetooth Debug Assistant](https://apps.apple.com/app/id1536579599)
  - [Android Bluetooth Debug Assistant](https://files.waveshare.com/wiki/common/Android_bluetooth_debug_assistant.zip)
  - [Android TCP Debug Assistant](https://dl.espressif.com/public/flash_download_tool.zip)
- Firmware Flashing Tool:
  - [Flash_download_tool](https://code.visualstudio.com/download)

## **5. Community Showcase**

:::info
Check out these amazing projects created by the community! Please note these are independently maintained by their creators.
:::

### That Project -The Ultimate Animated Face for ESP32 | 17+ Expressions

import YoutubeEmbed from '@site/src/components/YoutubeEmbed';

<YoutubeEmbed id="iCm50dTcAtc" maxWidth="700px" />

- Youtube: https://www.youtube.com/watch?v=iCm50dTcAtc
- Github: https://github.com/0015/lvgl_kawaii_face