---
sidebar_position: 1
title: Resources
slug: /ESP32-C3-Zero/Resources-And-Documents
product_family:
  - ESP32
product_model:
  - ESP32-C3-Zero
  - ESP32-C3-Zero-M
  - ESP32-C3-Zero-Basic-Kit
---

# Resources

## **1. Hardware Documentation**

**Development Board Design Files**

- [Schematic](https://www.waveshare.net/w/upload/d/dd/ESP32-C3-Zero-Sch.pdf)

## **2. Technical Manuals**

- **ESP32-C3 Chip Manuals**

  - Datasheet: [Chinese Version](https://documentation.espressif.com/esp32-c3_datasheet_cn.pdf) | [English Version](https://documentation.espressif.com/esp32-c3_datasheet_en.pdf)
  - Technical Reference Manual: [Chinese Version](https://documentation.espressif.com/esp32-c3_technical_reference_manual_cn.pdf) | [English Version](https://documentation.espressif.com/esp32-c3_technical_reference_manual_en.pdf)

- **Datasheets**
  - [WS2812B RGB LED Datasheet](https://files.waveshare.com/wiki/ESP32-C3-Zero/XL-0807RGBC-WS2812B.pdf)

## **3. Software Development Resources**

- **Example Code**

  - ESP32-C3-Zero: [Esp32-c3-zero-code.zip](https://files.waveshare.com/wiki/ESP32-C3-Zero/Esp32-c3-zero-code.zip)

  - ESP32-C3-Zero-Basic-Kit:
    - [ESP32-C3-Zero-Basic-Kit-main.zip](https://github.com/waveshareteam/ESP32-C3-Zero-Basic-Kit/archive/refs/heads/main.zip)
    - GitHub Repository: [waveshareteam/ESP32-C3-Zero-Basic-Kit](https://github.com/waveshareteam/ESP32-C3-Zero-Basic-Kit)

- **Debugging Tools**

  - [Serial and Network Debugging Assistant](https://files.waveshare.com/wiki/common/Sscom5.13.1.zip)